	...
	for _, zone := range zones {
		...
		for _, ns := range nsc {
			...
			ns.metaNodes.Range(func(key, value interface{}) bool {
				...
				nsView.MetaNodes = append(nsView.MetaNodes, proto.NodeView{ID: metaNode.ID, Addr: metaNode.Addr, Status: metaNode.IsActive, IsWritable: metaNode.isWritable(proto.StoreModeMem)})
				return true
			})
		}
	}

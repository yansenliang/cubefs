...
func erroutStrToErr(format string, a ...interface{}) {
	errout(fmt.Errorf(format, a...))
}
...
func newConvertNodeList(client *convert.ConvertClient) *cobra.Command {
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if  err != nil {
				erroutStrToErr("Get cluster info fail:\n%v\n", err)
			}
			stdout("[ConvertNode]\n")
	...
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if objType != "cluster" && objType != "task" {
				erroutStrToErr("unknown object type")
			}

			if len(optnodesAddrs) == 0 && len(optVolName) == 0 {
				erroutStrToErr("At least contain one param")
			}
			...
			if err != nil {
				erroutStrToErr("add %s failed case:\n%v\n", objType, err)
			}
...
func newConvertDelCmd(client *convert.ConvertClient) *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				erroutStrToErr("del failed case:\ndel type:%v\nerror:%v\n", objType, err)
			}
...
func newConvertInfoCmd(client *convert.ConvertClient) *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				erroutStrToErr("query %s failed case:\n%v\n", objType, err)
			}
...
func newConvertStartCmd(client *convert.ConvertClient) *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				erroutStrToErr("start %s failed case:\n%v\n", objType, err)
			}
...
func newConvertStopCmd(client *convert.ConvertClient) *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				erroutStrToErr("stop %s failed case:\n%v\n", objType, err)
			}

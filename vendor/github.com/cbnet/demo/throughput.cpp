...
DEFINE_uint32(msg_size, 16, "msg_size");
DEFINE_uint32(loop_count, -1, "loop count");
...
void start_client(int index) {
    ...
    while(ctx->running) {
        int sec = 1;
        int pps = (ctx->send_count - ctx->old_send_count) * 2 / sec;
        double tps = (ctx->send_bytes - ctx->old_send_bytes) * 1.0 / 1024 / 1024 / sec;
        uint64_t total_mb = ctx->send_bytes / 1024 / 1024;
        stringstream ss;
        ss << " msg_size:" << FLAGS_msg_size
           << " total_pkg:" << ctx->send_count << " total_bytes:" << total_mb << "MB"
           << " pps: " << pps << " tps: " << tps << "MBps";
        LOG(INFO) << ss.str();
        ctx->old_send_count = ctx->send_count;
        ctx->old_send_bytes = ctx->send_bytes;
...
int main(int argc, char *argv[]) {
    ...
    if ( FLAGS_t == "server") {
        ...
        for ( int i = 0; i < FLAGS_clinum; ++i ) {
            ths.emplace_back(std::thread(start_client, i));
        }

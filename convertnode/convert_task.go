...
func (task *ConvertTask) mpGetMpDetailInfo() (err error) {
	...
	task.info.MPConvertState = proto.MPSelectMetaNode
	return
}

func (task *ConvertTask) mpSelectMetaNode() (err error) {
	defer func() {
		if err == nil {
			task.info.MPConvertState = proto.MPAddReplic
		}
	}()
...
func (task *ConvertTask) mpAddReplic() (err error) {
	defer func() {
		if err == nil {
			task.info.MPConvertState = proto.MPWaitsync
		}
	}()
	if task.isContinueLastTask && len(task.mpInfo.Replicas) > int(task.conf.mpReplicaNum) {
...
func (task *ConvertTask) mpRunOnce() (finished bool, err error) {
	...
	case proto.MPSelectMetaNode:
		err = task.mpSelectMetaNode()
	case proto.MPAddReplic:
		err = task.mpAddReplic()
	case proto.MPWaitsync:
		err = task.mpWaitSync()

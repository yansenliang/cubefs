...
func (ie *InodeEx) Marshal() (data []byte, err error) {
	buff := bytes.NewBuffer(make([]byte, 0))

	bs, err := ie.Inode.Marshal()
	if err != nil {
		return nil, err
	}
...
func (ie *InodeEx) UnMarshal(raw []byte) (err error) {
	...
	ex := &Extend{}
	ex, err = NewExtendFromBytes(data)
	if err != nil {
		return
	}
